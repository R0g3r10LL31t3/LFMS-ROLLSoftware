...
@RequestScoped
@Path("/db/costfeature")
public class CostFeatureService
        extends AbstractServiceFacade<CostFeature, CostFeature.ObjectDataPK> {

    //@PersistenceContext(unitName = "LFMSPU")
    @Inject
    private CostFeatureRepository repo;

    @Inject
    @Synchronization(SyncType.SYNC)
    private EntityManager em;

    public CostFeatureService() {
    }

    public CostFeatureService(CostFeatureRepository repo, EntityManager em) {
        this.repo = repo;
        this.em = em;
    }

    @Override
    public Repository<CostFeature, CostFeature.ObjectDataPK, String>
            getRepository() {
        return repo;
    }
