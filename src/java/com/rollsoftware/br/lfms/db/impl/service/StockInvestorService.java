...
//@javax.ejb.Stateless
@RequestScoped
@Path("/db/stockinvestor")
public class StockInvestorService
        extends AbstractServiceFacade<StockInvestor, StockInvestor.ObjectDataPK> {

    //@PersistenceContext(unitName = "LFMSPU")
    @Inject
    private StockInvestorRepository repo;

    @Inject
    @Synchronization(SyncType.SYNC)
    private EntityManager em;

    public StockInvestorService() {
    }

    public StockInvestorService(StockInvestorRepository repo, EntityManager em) {
        this.repo = repo;
        this.em = em;
    }

    @Override
    public Repository<StockInvestor, StockInvestor.ObjectDataPK, String>
            getRepository() {
        return repo;
    }
