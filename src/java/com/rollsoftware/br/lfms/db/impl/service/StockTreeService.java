...
@RequestScoped
@Path("/db/stocktree")
public class StockTreeService
        extends AbstractServiceFacade<StockTree, StockTree.ObjectDataPK> {

    //@PersistenceContext(unitName = "LFMSPU")
    @Inject
    private StockTreeRepository repo;

    @Inject
    @Synchronization(SyncType.SYNC)
    private EntityManager em;

    public StockTreeService() {
    }

    public StockTreeService(StockTreeRepository repo, EntityManager em) {
        this.repo = repo;
        this.em = em;
    }

    @Override
    public Repository<StockTree, StockTree.ObjectDataPK, String>
            getRepository() {
        return repo;
    }
