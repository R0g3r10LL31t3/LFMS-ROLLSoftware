...
//@javax.ejb.Stateless
@RequestScoped
@Path("/db/variablecost")
public class VariableCostService
        extends AbstractServiceFacade<VariableCost, VariableCost.ObjectDataPK> {

    //@PersistenceContext(unitName = "LFMSPU")
    @Inject
    private VariableCostRepository repo;

    @Inject
    @Synchronization(SyncType.SYNC)
    private EntityManager em;

    public VariableCostService() {
    }

    public VariableCostService(VariableCostRepository repo, EntityManager em) {
        this.repo = repo;
        this.em = em;
    }

    @Override
    public Repository<VariableCost, VariableCost.ObjectDataPK, String>
            getRepository() {
        return repo;
    }
